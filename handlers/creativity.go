...
func CreateThreadForMessageCreativity(s *discordgo.Session, m *discordgo.MessageCreate) {
...
func HandleMessageInCreativity(s *discordgo.Session, m *discordgo.MessageCreate) {
	if m.Author.ID == s.State.User.ID || m.ChannelID != DiscordBotConfigValues.Channels.CreativityChannel {
		return
	}
	chanName, _ := s.Channel(m.ChannelID)
	if len(m.Attachments) > 0 {
		log.Debug(m.Attachments)
